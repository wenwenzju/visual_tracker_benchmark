...
namespace pe_re_id
{
	...
	double SdalfPeReId::feature_match(SdalfFeature& g, double lr)
	{
		int probes = probe.size();
		double wd = 0.;
		for (int i = 0; i < probes; ++i)
		{
			wd += get_weight(i, probes, lr)*bhattacharyya(probe[i].whisto2.whisto, g.whisto2.whisto);
		}
		return wd;
	}

	double SdalfPeReId::get_weight(int i, int n, double lr)
	{
		if (n == 1) return 1;
		if (i == 0) return 1-lr;
