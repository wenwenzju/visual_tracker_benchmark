...
%     [idx,C] = kmeans([x,y,s], k);
    ...
    img_name = sprintf('%04d.jpg', i);
    img = imread(['D:\data_seq\Walking2\' img_name]);
    ...
    for kk = 1:k
        %plot(C(kk,1), C(kk,2), '*','MarkerSize', 35, 'Color',[0,0,1]);
    end
...
%     td = randsample(1:length(x), length(x), true, exp(ss));
    ...
    edges = min([0 cumsum(exp(s)/sum(exp(s)))'],1); % protect against accumulated round-off
