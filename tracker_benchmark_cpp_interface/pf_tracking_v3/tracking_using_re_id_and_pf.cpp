...
PTUsingReIdandPF::PTUsingReIdandPF(int states /* = 3 */, int particles /* = 52 */, bool uofl /* = true */, bool uo /* = true */, double lr /* = 0.4 */, double sx /* = 20 */, 
	...
	,hog_scores_("hog_scores.txt"), sdalf_scores_("sdalf_scores.txt")/////////////////////////////////////////////
{
...
void PTUsingReIdandPF::init(cv::Mat& first_img, cv::Rect& first_loc)
{
	...
	/////////////////////////////////linear combination of template of each particle
	for (int i = -1; i < particles_num_; ++i)
	{
		vec_d uk,x;
		if (i == -1) rect2states(init_loc, x);
		else
		do
		{
		...
		cv::Rect tmp;
		tmp = r&cv::Rect(0,0,first_img.cols, first_img.rows);

		cv::resize(first_img(tmp), g[0], cv::Size(64, 128));
		std::vector<pe_re_id::SdalfFeature> gallery_feature;
		sdalf.gallery_feautre_extract(g, gallery_feature);
		par_fea_.push_back(gallery_feature[0]);
		//std::cout << gallery_feature[0].whisto2.whisto << std::endl;
		//system("pause");
	}
	vec_d gt; rect2states(init_loc, gt);
	calc_template_(par_fea_, ptcs_, gt, init_probe);
	sdalf.probe.push_back(init_probe);
	...
	for (int i = 0; i < particles_per_thread; s_xk++, s_pro++, s_item0++, s_item1++, i++)
	{
		...
		*s_item1 = det_coeff;
		...
		////////////////////////////////////////////
		{
			...
			sdalf_scores_ << d << " " << (*s_xk)[0] << " " << (*s_xk)[1] << " " << (*s_xk)[2] << std::endl;
			par_fea_.push_back(gallery_feature[0]);
			ptcs_.push_back(*s_xk);
		}
	}
}
...
cv::Rect PTUsingReIdandPF::track(cv::Mat& frame)
{
	...
	else
	{
		...
		calc_template_(par_fea_, ptcs_, xk, gallery_feature[0]);
...
void PTUsingReIdandPF::sdalf_scores_mapping_(vec_d& scores)
{
	...
	////std::for_each(scores.begin(), scores.end(), [&s, coeff](double& d){d = exp(coeff*d);s+=d;});
	...
	std::for_each(scores.begin(), scores.end(), [maxs, mins](double& d){d = exp(EXP_COEFF_HOG*(d-mins)/(maxs-mins));});
	normalize_(scores);
}

void PTUsingReIdandPF::calc_template_(std::vector<pe_re_id::SdalfFeature>& pf, std::vector<vec_d >& pt, vec_d& gt, pe_re_id::SdalfFeature& tpl)
{
	int rowH = pf[0].whisto2.whisto.rows * pf[0].whisto2.whisto.cols;
	int colH = pf.size();
	//cv::Mat H(rowH, colH, CV_64FC1);
	//cv::Mat Ov(colH, 1, CV_64FC1);
	//cv::Rect gt_rect;
	//states2rect(gt, gt_rect);
	//for (int i = 0; i < colH; ++i)
	//{
	//	pf[i].whisto2.whisto.col(0).copyTo(H.col(i));
	//	cv::Rect pt_rect;
	//	states2rect(pt[i], pt_rect);
	//	Ov.at<double>(i,0) = overlap_(gt_rect, pt_rect);
	//}

	//cv::Mat a = (H.t()*H).inv()*Ov;
	////cv::Scalar s = cv::sum(a);
	////cv::Mat tmp = H*a/s[0];
	//cv::Mat tmp = H*a;
	//tmp.copyTo(tpl.whisto2.whisto);

	Eigen::MatrixXd H(rowH, colH);
	Eigen::MatrixXd Ov(colH, 1);
	cv::Rect gt_rect;
	states2rect(gt, gt_rect);
	for (int i = 0; i < colH; ++i)
	{
		Eigen::MatrixXd tmpcol(rowH, 1);
		cv2eigen(pf[i].whisto2.whisto, tmpcol);
		H.col(i) = tmpcol;
		cv::Rect pt_rect;
		states2rect(pt[i], pt_rect);
		Ov(i) = overlap_(gt_rect, pt_rect);
	}
	H = H.array().sqrt();
	Ov = 1.-Ov.array().square();

	Eigen::MatrixXd HtH = H.transpose()*H;
	//Eigen::MatrixXd HtHi = HtH.inverse();
	Eigen::MatrixXd a = HtH.llt().solve(Ov);
	Eigen::MatrixXd tmp = H*a;
	tmp = tmp.array().square();
	eigen2cv(tmp, tpl.whisto2.whisto);

	//std::ofstream savepf("pf.txt"), savept("pt.txt"), savegt("gt.txt"), savetpl("tpl.txt"), saveov("ov.txt"), saveov1("calc_ov.txt");
	//for (int i = 0; i < rowH; ++i)
	//{
	//	//double* rowiptr = H.ptr<double>(i);
	//	for (int j = 0;j < colH-1; ++j)
	//	{
	//		//savepf << rowiptr[j] << " ";
	//		savepf << H(i,j) << " ";
	//	}
	//	//savepf << rowiptr[colH-1] << std::endl;
	//	savepf << H(i,colH-1) << std::endl;
	//}

	//for (int i = 0; i < pt.size(); ++i)
	//{
	//	savept << pt[i][0] << " " << pt[i][1] << " " << pt[i][2] << std::endl;
	//}

	//savegt << gt[0] << " " << gt[1] << " " << gt[2] << std::endl;

	//for (int i = 0; i < rowH; ++i)
	//{
	//	//savetpl << tmp.at<double>(i,0) << std::endl;
	//	savetpl << tmp(i) << std::endl;
	//}

	//for (int i = 0; i < colH; ++i)
	//{
	//	//saveov << Ov.at<double>(i,0) << std::endl;
	//	saveov << Ov(i) << std::endl;
	//}

	//Eigen::MatrixXd cOv = H.transpose()*tmp;
	//for (int i = 0; i < colH; ++i)
	//{
	//	saveov1 << cOv(i) << std::endl;
	//}

	//savepf.close();savept.close();savegt.close();savetpl.close();saveov.close();

	////std::cout << H << std::endl;
	////system("pause");
	////std::cout << H.transpose()*H << std::endl;
	////system("pause");
	////std::cout << (H.transpose()*H).inverse() << std::endl;
