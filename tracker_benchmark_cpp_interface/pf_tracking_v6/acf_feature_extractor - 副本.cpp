...
void AcfFeatureExtractor::feature_extract(cv::Mat& img)
{
	...
	//compute gradient histogram channels
	{
		...
		cv::Mat mH; H.copytoMat(mH);

		//cv::Mat L,U,V;
		//luv.copytoMat(L, 0);
		//L.convertTo(L, CV_8UC1, 255./0.37);
		//imshow("color L", L);
		//luv.copytoMat(U, 1);
		//U.convertTo(U, CV_8UC1, 255);
		//imshow("color U", U);
		//luv.copytoMat(V, 2);
		//V.convertTo(V, CV_8UC1, 255./0.89);
		//imshow("color V", V);
		...
		//{
		//	cv::Mat img;
		//	cv::normalize(chs[i], img, 0, 255, CV_MINMAX);
		//	cv::Mat img1;
...
double AcfFeatureExtractor::distance(MatchModel& mm1, MatchModel& mm2)
{
	...
	double s = 0.;
	double luv_weight = 0.8/luv_cnt;
	double cell_weight = 0.2/hog_cnt;
