...
PTUsingReIdandPF::PTUsingReIdandPF(int states /* = 3 */, int particles /* = 52 */, bool uofl /* = true */, bool uo /* = true */, double lr /* = 0.4 */, double sx /* = 20 */, 
	double sy /* = 10 */, double sw /* = 8 */, double ecri /* = 0.5 */, double riw /* = 0.8 */, double hpx /* = 1.2 */, int tn /* = 4 */, double ar /* = 0.43*/, double rwr /* = 0.5*/, int pn /* = 5*/)
	...
	,random_walk_ratio_(rwr), probe_needs_(pn)
#ifdef SAVE_SCORES_
	...
	int particles_per_thread)
{
	for (int i = 0; i < particles_per_thread; s_xk++, s_pro++, s_item0++, s_item1++, i++)
	{
		...
		d = sdalf.feature_match(gallery_feature[0], 0.5);
#else
		d = sdalf.feature_match(gallery_feature[0], learning_rate_);
#endif
...
void PTUsingReIdandPF::sample_from_q(std::vector<vec_d >& pold, std::vector<vec_d >& pnew)
{
	...
	if (is_fast_motion(lk_points_[0], lk_points_[1])) random_walk_ratio_ = 0.2;
...
// 	else {learning_rate_ = 0.4;}
	...
	if (!update_online_)
	{
		if (!sdalf.probe_ready)
		{
			if (sdalf.probe.size() == probe_needs_) {sdalf.save_probe();frame_num = 4;}
			if (frame_num == 3)
			{
				frame_num = 0;
				cv::Mat tmp;
	...
	else
	{
		...
		re_id_model_update(gallery_feature[0], sdalf.probe);
...
void PTUsingReIdandPF::rw_motion_update(vec_d& particle_pre, vec_d& particle_cur, double w_star, double vx_star, double vy_star, int frame_w, int frame_h)
{
	...
	particle_cur[2] = w_star + gen_gaussian_noise(0,width_star_/10);
	if (particle_cur[2] < 10) particle_cur[2] = 10;
	...
	else particle_cur[3] = gen_gaussian_noise(0, 5);
...
#define FAST_MOTION_DISPLACEMENT_THRESHOLD 10
	...
	if (displacement > FAST_MOTION_DISPLACEMENT_THRESHOLD || displacement > width_star_/3) return true;
	return false;
}