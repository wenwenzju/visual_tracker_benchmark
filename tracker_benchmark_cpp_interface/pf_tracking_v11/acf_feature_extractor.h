...
struct MatchModel
{
	...
	WWMatrix<double> chn_color;			//(binl+binu+binv)x1
	std::vector<WWMatrix<double> > hog;	//(cellswxcellsh+1)x(binhogx1)
	WWMatrix<double> chn_go;			//(cellswxcellshxbinhog)x1
};
...
private:
	...
	void feature_hist(std::vector<cv::Mat>& integ_orient, cv::Rect& roi, double* hist, int bin = 0);
