...
{
	return 0.7*bhattacharyya(mm1.chn_color.data.get(), mm2.chn_color.data.get(), binl+binu+binv)
		+0.3*bhattacharyya(mm1.chn_go.data.get(), mm2.chn_go.data.get(), cellsh*cellsw*binhog);
	std::vector<double> d;
	int luv_cnt = 0, hog_cnt = 0;
	double dis = 0;

	dis = bhattacharyya(mm1.chnL.data.get(), mm2.chnL.data.get(), binl);
	if (dis < 10. && dis >= 0.)
	{d.push_back(dis);luv_cnt++;}				//distance of channel L feature

	dis = bhattacharyya(mm1.chnU.data.get(), mm2.chnU.data.get(), binu);
	if (dis < 10. && dis >= 0.)
	{d.push_back(dis);luv_cnt++;}				//distance of channel U feature

	dis = bhattacharyya(mm1.chnV.data.get(), mm2.chnV.data.get(), binv);
	if (dis < 10. && dis >= 0.)
	{d.push_back(dis);luv_cnt++;}				//distance of channel V feature
	
	for (int i = 0; i < cellsh*cellsw+1; ++i)
	{
		double tmpd = bhattacharyya(mm1.hog[i].data.get(), mm2.hog[i].data.get(), binhog);
		if (tmpd > 10. || tmpd < 0.) continue;
		d.push_back(tmpd);	//distance of hog per cell in particle
		hog_cnt++;
	}

	double s = 0.;
	double luv_weight = 0.7/luv_cnt;
	double cell_weight = 0.3/hog_cnt;
	std::for_each(d.begin(), d.begin()+luv_cnt, [&s, luv_weight](double d){s += luv_weight*d;});
	std::for_each(d.begin()+luv_cnt, d.end(), [&s, cell_weight](double d){s += cell_weight*d;});
	return s;
}
...
{
	//mmroi.chnL.creat(binl,1);
	//feature_hist(luv, roi, 0, binl, MINL, MAXL, mmroi.chnL.data.get());

	//mmroi.chnU.creat(binu, 1);
	//feature_hist(luv, roi, 1, binu, MINU, MAXU, mmroi.chnU.data.get());

	//mmroi.chnV.creat(binv, 1);
	//feature_hist(luv, roi, 2, binv, MINV, MAXV, mmroi.chnV.data.get());

	mmroi.chn_color.creat(binl+binu+binv,1);
	feature_hist(luv, roi, 0, binl, MINL, MAXL, mmroi.chn_color.data.get());
	feature_hist(luv, roi, 1, binu, MINU, MAXU, mmroi.chn_color.data.get()+binl);
	feature_hist(luv, roi, 2, binv, MINV, MAXV, mmroi.chn_color.data.get()+binl+binu);
	mmroi.chn_go.creat(cellsw*cellsh*binhog,1);

	//WWMatrix<double> cell0(binhog, 1);
	//feature_hist(orients, roi, cell0.data.get());
	//mmroi.hog.push_back(cell0);
	...
	for (int i = 0; i < cellsw; ++i)
	{
		...
		for (int j = 0; j < cellsh; ++j)
		{
			...
			//WWMatrix<double> celli(binhog, 1);
			cv::Rect r(roi.x+i*cell_width, roi.y+j*cell_height, cw, ch);
			//feature_hist(orients, r, celli.data.get());
			feature_hist(orients, r, mmroi.chn_go.data.get()+cnt*binhog, binhog);
			//mmroi.hog.push_back(celli);
			cnt++;
		}
	}
}
...
void AcfFeatureExtractor::feature_hist(std::vector<cv::Mat>& integ_orient, cv::Rect& roi, double* hist, int bin/* = 0*/)
{
	int chns = bin?bin:integ_orient.size();
	for (int i = 0; i < chns; ++i)
	{
