...
namespace pe_re_id
{
	...
	int SdalfPeReId::feature_match(std::vector<SdalfFeature>& g, cv::Mat& M, double* min_dis /* = NULL */)
	{
		std::vector<double> final_dist_hist;

		int probes = probe.size(), gallerys = g.size();
		for (int j = 0; j < gallerys; ++j)
		{
			double tmp = FLT_MAX;
			for (int ii = 0; ii < probes; ++ii)
			{
				double b = mahalanobis(probe[ii].whisto2.whisto, g[j].whisto2.whisto, M);
				if (tmp > b) tmp = b;
			}
			final_dist_hist.push_back(tmp);
		}
