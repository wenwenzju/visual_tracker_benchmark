...
PTUsingReIdandPF::PTUsingReIdandPF(int states /* = 3 */, int particles /* = 52 */, bool uofl /* = true */, bool uo /* = true */, double lr /* = 0.4 */, double sx /* = 20 */, 
	...
	,input_(particles*2, 108), labels_(particles*2), valid_inputs_(0), template_pool_full_(false), sdalf_scores_("sdalf_scores.txt")
{
	sigma_[0] = sx;
	sigma_[1] = sy;
	sigma_[2] = sw;
...
void PTUsingReIdandPF::init(cv::Mat& first_img, cv::Rect& first_loc)
{
	...
	for (int i = 0; i < particles_num_; ++i)
	{
		...
		Eigen::Matrix<double, 1, 108> tmprow;
		cv2eigen(gallery_feature[0].whisto2.whisto, tmprow);
		input_.row(valid_inputs_) = tmprow;
		if (overlap_(r, previous_loc) >= 0.65) labels_(valid_inputs_) = 1;
		else labels_(valid_inputs_) = 0;
		valid_inputs_++;
	}
	Eigen::MatrixXd aa;
	nca::nca(input_.topRows(valid_inputs_), labels_.topRows(valid_inputs_), 0, 0, aa);
	aa.resize(input_.cols(),aa.rows()*aa.cols()/input_.cols());
	aa = aa*aa.adjoint();
	eigen2cv(aa, M_);
	first_frame_valid_inputs_ = valid_inputs_-1;
}
	...
	int particles_per_thread)
{
	for (int i = 0; i < particles_per_thread; s_xk++, s_pro++, s_item0++, s_item1++, i++)
	{
		...
		sdalf.gallery_feautre_extract(g, gallery_feature);
		double d;
		sdalf.feature_match(gallery_feature, M_ ,&d);
...
//#define MIN_MAX
	...
	for (int i = 0; i < pn; ++i)
		pro[i] = re_id_weight_*item0[i]/si0 + (1-re_id_weight_)*item1[i]/si1;
}
...
cv::Rect PTUsingReIdandPF::track(cv::Mat& frame)
{
	...
	//distance metric learning about, update
	for (int i = 0; i < par_fea_.size(); ++i)
	{
		Eigen::Matrix<double, 1, 108> tmprow;
		cv2eigen(par_fea_[i].whisto2.whisto, tmprow);
		input_.row(valid_inputs_) = tmprow;
		cv::Rect r;
		states2rect(ptcs_[i], r);
		if (overlap_(r, previous_loc) >= 0.65) labels_(valid_inputs_) = 1;
		else labels_(valid_inputs_) = 0;
		valid_inputs_++;
		if (valid_inputs_ >= input_.rows()) {valid_inputs_ = first_frame_valid_inputs_+1;template_pool_full_ = true;}
	}
	par_fea_.clear();
	Eigen::MatrixXd aa;
	if (template_pool_full_)
		nca::nca(input_, labels_, 0, 0, aa);
	else
		nca::nca(input_.topRows(valid_inputs_), labels_.topRows(valid_inputs_), 0, 0, aa);
	aa.resize(input_.cols(),aa.rows()*aa.cols()/input_.cols());
	aa = aa*aa.adjoint();
