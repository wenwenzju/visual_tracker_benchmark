...
namespace pe_re_id
{
	...
	public:
		...
		int feature_match(std::vector<SdalfFeature>& g, cv::Mat& M, double* min_dis = NULL);
	...
	//private:
		...
		inline double mahalanobis(const cv::Mat& k, const cv::Mat& q, const cv::Mat& M){return (cv::Mat(k.t()*M*q)).at<double>(0,0);};
