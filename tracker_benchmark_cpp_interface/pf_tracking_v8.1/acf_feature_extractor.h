...
struct MatchModel
{
	...
	WWMatrix<double> features;			//(binl+binu+binv+binhog)x(cellsw x cellsh)
};
...
private:
	...
	inline void normalize(double* d, int l) {double s = 0.; for (int i = 0; i < l; ++i) s+=d[i];if (s==0.) return; for (int i = 0; i < l; ++i) d[i]/=s;}
