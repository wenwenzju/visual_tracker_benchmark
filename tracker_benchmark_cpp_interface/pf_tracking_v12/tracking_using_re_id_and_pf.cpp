...
PTUsingReIdandPF::PTUsingReIdandPF(int states /* = 6 */, int particles /* = 52 */, bool uofl /* = true */, bool uo /* = true */, double lr /* = 0.4 */, double sx /* = 20 */, 
	...
	: ParticleFilter(states, particles), acf_extractor(binL, binU, binV, binHOG, 4, 2, init_probe, cur_probe), /*rng(time(0)),*/ no_matched_orb(false),
	termcrit_(CV_TERMCRIT_ITER|CV_TERMCRIT_EPS, 20, 0.03), sub_pix_winSize(10,10), win_size(31,31), MAX_COUNT(500),
...
#ifdef SAVE_SCORES_
	,match_scores_("match_scores.txt")
#endif
{
...
// 	}
	...
	sigma_[2] = 1.*previous_loc.width/20. < sigma_max_[2] ? 1.*previous_loc.width/20. : sigma_max_[2];
	sigma_[3] = 1.*previous_loc.height/20. < sigma_max_[2] ? 1.*previous_loc.height/20. : sigma_max_[2];
}
...
void PTUsingReIdandPF::gen_x0(vec_d& x0)
{
	...
	x0.push_back(x[3] + gen_gaussian_noise(0, sigma_[3]));//height
	...
	if (x0[2] < MIN_WIDTH) x0[2] = MIN_WIDTH;
	else if (x0[2] > pre_frame.cols) x0[2] = pre_frame.cols;
	if (x0[3] < MIN_HEIGHT) x0[3] = MIN_HEIGHT;
	else if (x0[3] > pre_frame.rows) x0[3] = pre_frame.rows;
}
...
{
	for (int i = 0; i < particles_per_thread; s_xk++, s_pro++, s_item0++, i++)
	{
		cv::Rect r;
		states2rect(*s_xk, r);
		...
		double d = acf_extractor.calc_roi_scores(tmp, learning_rate_);
		{
			...
			if (d > 1000.) std::cout<<"x: "<<tmp.x<<"; y: "<<tmp.y<<"; w: "<<tmp.width<<"; h: "<<tmp.height<<"; distance: "<<d<<std::endl;
		}
...
//#define MIN_MAX
	...
	for (int i = 0; i < thread_num_; ++i)
	{
		...
		if (i != thread_num_-1)
			...
			s_xk, s_pro, s_item0, &si0, particles_per_thread));
		else
		{
			threads[i] = boost::thread(boost::bind(&PTUsingReIdandPF::p_yk_given_xk_multi_thread, this, 
				s_xk, s_pro, s_item0, &si0, particles_per_thread+particles_num_%thread_num_));
		}
	}
...
void PTUsingReIdandPF::sample_from_q(std::vector<vec_d >& pold, std::vector<vec_d >& pnew)
{
	...
	static double rwr = random_walk_ratio_; random_walk_ratio_ = rwr;
	if (is_fast_motion(lk_points_[0], lk_points_[1])) random_walk_ratio_ = 0.1;
	...
	for (; fpt_ite_new != pnew.end(); fpt_ite_new++)
	{
		...
		(*fpt_ite_new)[3] = cp[3]+gen_gaussian_noise(0, sigma_[3]);
		(*fpt_ite_new)[4] = vx_star_+gen_gaussian_noise(0,2);
		(*fpt_ite_new)[5] = vy_star_+gen_gaussian_noise(0,2);
		...
		if ((*fpt_ite_new)[2] < MIN_WIDTH) (*fpt_ite_new)[2] = MIN_WIDTH;
		else if ((*fpt_ite_new)[2] > pre_frame.cols) (*fpt_ite_new)[2] = pre_frame.cols;

		if ((*fpt_ite_new)[3] < MIN_HEIGHT) (*fpt_ite_new)[3] = MIN_HEIGHT;
		else if ((*fpt_ite_new)[3] > pre_frame.rows) (*fpt_ite_new)[3] = pre_frame.rows;
	}
	///////////////////////////////////////////////////////////////////////
	...
	{
		rw_motion_update(*rw_ite_old, *rw_ite_new, width_star_, height_star_, vx_star_, vy_star_, pre_frame.cols, pre_frame.rows);
	}
...
// 	else {learning_rate_ = 0.4;}
	...
	height_star_ = filter_height(xk[3]);
	...
	sigma_[3] = 1.*previous_loc.height/10. < sigma_max_[3] ? 1.*previous_loc.height/10. : sigma_max_[3];
...
void PTUsingReIdandPF::states2rect(vec_d& x, cv::Rect& r)
{
	int xx = x[0] - x[2]/2;
	int yy = x[1] - x[3]/2;
	r = cv::Rect(xx,yy,(int)x[2],(int)x[3]);
}
...
void PTUsingReIdandPF::fpt_motion_update(vec_d& particle_pre, vec_d& particle_cur, double meanx, double meany)
{
	...
	if (particle_cur[2] < MIN_WIDTH) particle_cur[2] = MIN_WIDTH;
...
void PTUsingReIdandPF::rw_motion_update(vec_d& particle_pre, vec_d& particle_cur, double w_star, double h_star, double vx_star, double vy_star, int frame_w, int frame_h)
{
	//center x
	double xtmp = particle_pre[0]+particle_pre[4];
	if (xtmp < 0)
		particle_cur[0] = xtmp+gen_uniform_noise(0,w_star/4);
	else if (xtmp > frame_w)
		particle_cur[0] = xtmp-gen_uniform_noise(0, w_star/4);
	else
		particle_cur[0] = xtmp+gen_gaussian_noise(0,w_star/10);
	if (particle_cur[0] < 0) particle_cur[0] = 0;
	else if (particle_cur[0] > frame_w) particle_cur[0] = frame_w;

	//center y
	double ytmp = particle_pre[1] + particle_pre[5];
	if (ytmp < 0)
		particle_cur[1] = ytmp+gen_uniform_noise(0,h_star/4);
	else if (ytmp > frame_h)
		particle_cur[1] = ytmp-gen_uniform_noise(0,h_star/4);
	else
		particle_cur[1] = ytmp+gen_gaussian_noise(0,h_star/10);
	if (particle_cur[1] < 0) particle_cur[1] = 0;
	else if (particle_cur[1] > frame_h) particle_cur[1] = frame_h;

	//width
	/*particle_cur[2] = particle_pre[2] + gen_gaussian_noise(0,3);*/
	particle_cur[2] = w_star + gen_gaussian_noise(0,w_star/20);
	if (particle_cur[2] < MIN_WIDTH) particle_cur[2] = MIN_WIDTH;
	else if (particle_cur[2] > frame_w) particle_cur[2] = frame_w;

	//height
	particle_cur[3] = h_star + gen_gaussian_noise(0, h_star/20);
	if (particle_cur[3] < MIN_HEIGHT) particle_cur[3] = MIN_HEIGHT;
	else if (particle_cur[3] > frame_h) particle_cur[3] = frame_h;

	if (particle_pre[0]< w_star/2) particle_cur[4] = particle_pre[4]+gen_uniform_noise(0,w_star);
	else if(particle_pre[0] > frame_w-w_star/2) particle_cur[4] = particle_pre[4]-gen_uniform_noise(0,w_star);
	...
	else particle_cur[4] = gen_gaussian_noise(0, 5);
	//else particle_cur[3] = gen_gaussian_noise(0, w_star/3);

	if (particle_pre[1]< h_star/2) particle_cur[5] = particle_pre[5]+gen_uniform_noise(0,h_star);
	else if(particle_pre[1] > frame_h-h_star/2) particle_cur[5] = particle_pre[5]-gen_uniform_noise(0,h_star);
	...
	else particle_cur[5] = gen_gaussian_noise(0, 5);
...
double PTUsingReIdandPF::filter_height(double d)
{
	static int t = 1;
	static double pt_1 = 0., et = 0, sumet = 0., et_1 = 0.;
	static double Kp=0.2, Ki=0.003, Kd=0.03;
...
void PTUsingReIdandPF::drawMatches(cv::Mat& img1, std::vector<cv::Point2f>& points1, std::vector<vec_d>& ptc1, cv::Mat& img2, std::vector<cv::Point2f>& points2,std::vector<vec_d>& ptc2, cv::Mat& res)
{
	...
	for (;iteb1 != itee1; iteb1++,iteb2++)
	{
		...
		rectangle(res, r2.tl()+offset, r2.br()+offset-Point(1,1), Scalar(255,0,0));
	}
	...
	for (;iteb1 != itee1; iteb1++,iteb2++)
	{
		...
		rectangle(res, r2.tl()+offset, r2.br()+offset-Point(1,1), Scalar(0,0,255));
	}
}

bool PTUsingReIdandPF::is_fast_motion(std::vector<cv::Point2f>& p1, std::vector<cv::Point2f>& p2)
{
#define FAST_MOTION_DISPLACEMENT_THRESHOLD 10
	int n = p1.size();
	if (n <= 10) return false;
	cv::Point2f mp1, mp2;
	std::for_each(p1.begin(), p1.end(),[&mp1](cv::Point2f& p){mp1 += p;});
	std::for_each(p2.begin(), p2.end(),[&mp2](cv::Point2f& p){mp2 += p;});
	mp1*=1./n; mp2*=1./n;
	double displacement = cv::norm(mp1-mp2);
	//std::cout << "displacement: " << displacement << "	" << width_star_/4 << std::endl;
	if (displacement > FAST_MOTION_DISPLACEMENT_THRESHOLD || displacement > width_star_/3 || displacement > height_star_/3) return true;
	return false;
}