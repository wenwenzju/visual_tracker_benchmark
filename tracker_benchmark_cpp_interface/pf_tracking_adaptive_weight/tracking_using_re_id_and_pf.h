...
private:
	...
	///////////////////////////////////2016/10/17 adaptive weight in observation model
	void histogram_(const std::vector<vec_d >& p, double w, double h, vec_d& hist_x, vec_d& hist_y);
	bool multi_persons_(const vec_d& hist_x, const vec_d& hist_y, double threshold = 0.4, int bin_thresh = 3);
	vec_d detect_socres_;
	void resample_(std::vector<vec_d>& xk, vec_d& wk);
	...
	double self_adaptive_weight_(const std::vector<vec_d >& particles, const vec_d& cs_scores, const vec_d& ts_scores, const vec_d& gt);
};
