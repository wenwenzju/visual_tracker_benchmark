...
PTUsingReIdandPF::PTUsingReIdandPF(int states /* = 3 */, int particles /* = 52 */, bool uofl /* = true */, bool uo /* = true */, double lr /* = 0.4 */, double sx /* = 20 */, 
	...
	,hog_scores_("hog_scores.txt"), particle_features_("particle_features.txt"), sdalf_scores_("sdalf_scores.txt")/////////////////////////////////////////////
{
...
void PTUsingReIdandPF::init(cv::Mat& first_img, cv::Rect& first_loc)
{
	...
	{
		cv::Mat tmp;
		cv::resize(pre_frame(first_loc), tmp, cv::Size(64,128));
...
//#define MIN_MAX
	...
	vec_d hog_xk(states_num_,0), sdalf_xk(states_num_,0), mean_xk(states_num_, 0);
	for (int i = 0; i < xk.size(); ++i)
	{
		sdalf_xk[0] += item0[i]*xk[i][0];sdalf_xk[1] += item0[i]*xk[i][1];sdalf_xk[2] += item0[i]*xk[i][2];
		hog_xk[0] += item1[i]*xk[i][0];hog_xk[1] += item1[i]*xk[i][1];hog_xk[2] += item1[i]*xk[i][2];
		mean_xk[0] += 1./pn*xk[i][0];mean_xk[1] += 1./pn*xk[i][1];mean_xk[2] += 1./pn*xk[i][2];
	}
	states2rect(hog_xk, hog_res_);
	states2rect(sdalf_xk, sdalf_res_);
	states2rect(mean_xk, mean_res_);
	...
	double re_id_weight = re_id_weight_;
	//if (multi_persons_(hist_x, hist_y, 0.3)) {re_id_weight = 1;}
	//else {re_id_weight = re_id_weight_;}
	std::cout << "re_id_weight = " << re_id_weight << std::endl;

	for (int i = 0; i < pn; ++i)
		//pro[i] = re_id_weight*item0[i]/si0 + (1-re_id_weight)*item1[i]/si1;
		pro[i] = re_id_weight*item0[i] + (1-re_id_weight)*item1[i];
	//for (int i = 0; i < pn; ++i) pro[i] = item0[i]/si0 * item1[i]/si1;
	normalize_(pro);
}
...
void PTUsingReIdandPF::gen_sys_noise(vec_d& uk)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		boost::uniform_real<double> ur(-sigma_[i], sigma_[i]);
...
cv::Rect PTUsingReIdandPF::track(cv::Mat& frame)
{
	...
	//if (previous_loc.width >= init_loc.width*2 || previous_loc.width <= init_loc.width/2)
	//{
	//	sigma_[0] = 1.*previous_loc.width/2.;
	//	sigma_[1] = 1.*previous_loc.height/6.;
	//	sigma_[2] = 1.*previous_loc.width/6.;
	//}
	//sigma_[0] = 1.*previous_loc.width/3.;
	sigma_[0] = 1.*previous_loc.width/3. < sigma_max_[0] ? 1.*previous_loc.width/3. : sigma_max_[0];
	...
	cv::rectangle(frame, sdalf_res_, cv::Scalar(255,0,0));
	cv::rectangle(frame, hog_res_, cv::Scalar(0,255,0));
	cv::rectangle(frame, mean_res_, cv::Scalar(255,255,0));
...
//	std::for_each(scores_.begin(), scores_.end(), [maxs, mins](double& d){d = exp(EXP_COEFF_HOG*(d-mins)/(maxs-mins));});
	std::for_each(scores_.begin(), scores_.end(), [](double& d){d = exp(d);});
	normalize_(scores_);
}
...
double PTUsingReIdandPF::self_adaptive_weight_(const std::vector<vec_d >& particles, const vec_d& cs_scores, const vec_d& ts_scores, const vec_d& gt)
{
	int sn = particles[0].size();
	vec_d x_cs(sn, 0), x_ts(sn, 0);
	for (int i = 0; i < particles.size(); ++i)
	{
		for (int j = 0; j < sn; ++j)
		{
			x_cs[j] += cs_scores[i]*particles[i][j];
			x_ts[j] += ts_scores[i]*particles[i][j];
		}
	}

	double a = 0., num = 0., den = 0.;
	for (int i = 0; i < sn; ++i)
	{
		num += (x_ts[i]-x_cs[i])*(gt[i]-x_cs[i]);
		den += (x_ts[i]-x_cs[i])*(x_ts[i]-x_cs[i]);
	}

	return den == 0 ? 0.8 : num/den;
}