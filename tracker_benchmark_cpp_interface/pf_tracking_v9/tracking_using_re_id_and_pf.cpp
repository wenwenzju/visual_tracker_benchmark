...
void PTUsingReIdandPF::sample_from_q(std::vector<vec_d >& pold, std::vector<vec_d >& pnew)
{
	...
	//{
	//	cv::Rect r; states2rect(pnew[i], r);
	//	cv::imshow("particle_i", (*cur_frame)(r&cv::Rect(0,0,cur_frame->cols, cur_frame->rows)));
	//	cv::waitKey(0);
	//}
}
...
void PTUsingReIdandPF::drawMatches(cv::Mat& img1, std::vector<cv::Point2f>& points1, std::vector<vec_d>& ptc1, cv::Mat& img2, std::vector<cv::Point2f>& points2,std::vector<vec_d>& ptc2, cv::Mat& res)
{
	...
	vector<vec_d>::iterator iteb1 = ptc1.begin(), itee1 = ptc1.begin()+10*random_walk_ratio_,//+ptc1.size()*random_walk_ratio_,
		iteb2 = ptc2.begin(), itee2 = ptc2.begin()+10*random_walk_ratio_;//+ptc2.size()*random_walk_ratio_;
	...
	iteb1 = ptc1.begin()+ptc1.size()*random_walk_ratio_;
	iteb2 = ptc2.begin()+ptc2.size()*random_walk_ratio_;
	itee1 = iteb1 + 10*(1-random_walk_ratio_)+1;
	for (;iteb1 != itee1; iteb1++,iteb2++)
	{
		Rect r1, r2;
		states2rect(*iteb1, r1);
...
#define FAST_MOTION_DISPLACEMENT_THRESHOLD 10
	...
	if (displacement > FAST_MOTION_DISPLACEMENT_THRESHOLD || displacement > width_star_/2) return true;
	return false;
}